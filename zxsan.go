...
/*
 ZXSAN DDOS TOOL<strike>steroids</strike> 
*/
...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present zxsan will use POST requests instead of GET")
	...
	go func() {
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- ZXSAN Attack DONE --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
